...
export default function HeroSection() {
  return (
    <section
      className="flex flex-col items-center bg-white px-5 h-full overflow-hidden"
      style={{
        paddingTop: 'max(env(safe-area-inset-top, 0px) + 8px, 12px)',
        paddingBottom: 'max(env(safe-area-inset-bottom, 0px) + 8px, 16px)',
      }}
    >
      <div className="w-full max-w-[420px] md:max-w-[480px] flex flex-col items-center mx-auto flex-1 min-h-0">
        ...
        >
          <Image
            ...
            width={460}
            height={460}
            className="object-contain w-[260px] sm:w-[300px] md:w-[340px] h-auto"
            priority
          />
        </motion.div>
        ...
        {/* Card stack */}
        <motion.div
          className="w-full flex flex-col gap-2 -mt-3 flex-1 min-h-0"
          initial={{ opacity: 0, y: 14 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.7, delay: 0.16, ease: [0.25, 0.1, 0.25, 1] as const }}
        >
          {/* Berry cards */}
          <div className="flex w-full gap-2 shrink-0" style={{ height: 'clamp(220px, 38vh, 340px)' }}>
          ...
          {/* Divider */}
          <div className="shrink-0 w-full h-px bg-border my-1" />

          {/* Berry Preserve */}
          <div className="shrink-0 flex flex-col items-center justify-center px-4 py-2.5 rounded-xl bg-[#f9f9f8] border border-border">
            <p className="font-serif text-[20px] font-bold text-charcoal text-center tracking-widest">The Berry Preserve</p>
          </div>
          ...
          <div className="w-full grid grid-cols-2 gap-2.5 shrink-0">
            <a
              ...
              className="flex items-center gap-3 bg-[#25D366]/10 border border-[#25D366]/25 rounded-xl px-4 py-3 hover:bg-[#25D366]/18 transition-colors cursor-pointer"
            >
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-3 bg-[#E1306C]/10 border border-[#E1306C]/25 rounded-xl px-4 py-3 hover:bg-[#E1306C]/18 transition-colors cursor-pointer"
            >
